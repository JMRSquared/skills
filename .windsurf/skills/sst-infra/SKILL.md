---
name: sst-infra
description: Use when editing infra/**, sst.config.ts, or adding SST resources. Enforces stage-based configuration, SST secret management, single-region consistency, and defers to jmr-standing-rules for deployment safety.
---
...
- Standardise on the region your team has chosen (set once in config). Do not vary per resource. If a new resource needs a different region, flag it and confirm with the user before adding a second region.
...
- `jmr-standing-rules` rule #1 overrides this skill: **never run `sst deploy`** without explicit user confirmation in the current turn.
...
- Region matches the team-standard region; deviations are flagged.
