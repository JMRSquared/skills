---
name: knex-migration
description: Use when editing Knex migrations. Enforces ULID primary keys, snake_case columns, created_at/updated_at timestamps, and explicit foreign-key constraints.
---

# Knex Migration Standards

Apply when creating or editing a migration in your Knex migrations directory, or when adding a new table / column via Knex in any package.
...
- `yarn db:migrate:latest` (or your project's migrate command) applies cleanly against a fresh DB and against the current dev DB.
- `yarn db:migrate:rollback` reverts cleanly.
