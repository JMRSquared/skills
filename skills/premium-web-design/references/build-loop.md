...
| `contrast` | Text fails WCAG AA against its own composited background. **The check walks the DOM for the nearest painted background colour — it cannot see the pixels of a photograph.** It *can* now see an SVG `fill`: a badge built as an inline `<svg>` with a filled `<rect>` behind its text used to report 1:1, because SVG paints with `fill` and never sets `background-color`, and unlike the photograph case that one blocks the build. The nearest filled shape that actually covers the text is used instead. Over imagery it reports the section's ground and is close to meaningless: it will pass an eyebrow that has vanished into a lit lamp, and fail hero copy that is perfectly legible on a dark plate. Judge type-on-photo from the frame, not from this finding. **Tint marks are exempt:** a giant `aria-hidden="true"` letterform set a shade off its own ground is a deliberate device — tripletta's `WALLOVE` in sage, banzai's 1.2:1 marquee, blindbarber's year numerals — and the auditor skips it, counting it on the `ambition:` line as `tint marks n` instead. The exemption only applies when the same string also appears elsewhere as real readable content, so you cannot hide information inside an unreadable mark. | Darken/lighten the text, not the brand color. Gray-on-gray body copy (`#9ca3af` on white) is the single most common tell of an AI-built page. |
...
| `nav-illegible-over-content` | A fixed or sticky nav goes unreadable against what scrolls beneath it. Measured at 7 scroll positions by **hiding the bar and reading the ground from real pixels** — walking the DOM for it does not work, because a transparent wrapper chain sends the walk straight past the hero photograph to `<body>`. Three exemptions, all of them things that solve the problem by construction: an opaque plate (`alpha >= 0.85`) or a backdrop filter on the bar or on something inside it that covers it; `mix-blend-mode` anywhere from the link up to the bar, which inverts the ink against the ground (this is how Blind Barber's nav computes as white and paints black); and a `text-shadow`, which buys back 1.5 of the required ratio rather than exempting the link outright. A gradient scrim does **not** exempt: the usual one is transparent by the bottom edge, which is where the links sit. | Give the bar an opaque plate once it leaves the hero, swap its ink with the ground, or put `mix-blend-mode: difference` on it. A nav that reads over the hero and vanishes over the section that flips is invisible to every static check, including this skill's own frames, because a screenshot at scroll 0 looks correct. |
...
| `image-upscaled` | **Not what the name says, and the difference has cost a rebuild.** Under a `w`-descriptor `srcset` the browser density-corrects `naturalWidth`, so this compares *the width your `sizes` attribute claimed* against the rendered width — not file pixels against rendered pixels. Genuine 1200px files reported `natural: 806` and sent one build to `sips` to re-encode images that were already correct. **The fix is almost always in `sizes`, not in the file.** Write the phone breakpoint in: `sizes="(max-width: 720px) 34vw, 15vw"`. See "`sizes` must describe the phone" in `imagery.md`. Only when there is no `srcset` at all does this mean the source is genuinely too small. |
| `image-broken` | A real 404. Fires only when the browser attempted the request and got nothing (`complete && naturalWidth === 0`). | Fix the path. |
...
| `motion-uniform` | Counted from **distinct recipes** (property + duration + easing), not from element count. Recipes come from the stylesheet, plus a sweep of the computed styles of elements that actually animate. That sweep exists because a `transition` shorthand containing `var()` is a pending-substitution value: the CSSOM serialises the shorthand *and every longhand it covers* as the empty string, and `cssText` prints them back as `transition-property: ;`. Written the way this skill's own demos write it — `transition: transform var(--t-panel) var(--e-settle)` beside a `transition-delay` longhand — the declaration is unrecoverable from the rule, and one build added two real recipes, watched the finding refuse to move, and only found out by reading the auditor's source. The sweep widens the recipe set but contributes no element tallies, so a page whose motion is written **entirely** in var() shorthands can still be under-measured on the dominance figure. | Give entrances more than one register. If the finding will not move, check the recipes differ in property, duration or curve and not only in delay. |
...
| `technique-near-miss` (NOTE) | Not a finding. Lists the motion-technique candidates the probe found and rejected, and why — an unpainted canvas, a loader hook that never went away, a lone `view-transition-name`, a sticky stage with too little scroll room (with the ratio it missed by), a cursor preview that tracks the pointer from off-screen. | Read it when the technique count is one below where you expected. A one-technique swing on a geometry constant you can only find by reading the auditor is the same failure as a downgrade nobody logs. |
| `tint-mark-not-exempt` (NOTE) | A giant `aria-hidden` mark went into the contrast check instead of being exempted as a tint device. | The exemption needs the mark's text, normalised to letters and digits, to appear as a **whole token** inside some readable element. A giant `14` beside the words "fourteen weeks" does not match; beside a readable `14 weeks` it does. |
