...
Seven of this skill's fifteen studies are 3D or product pages and none told
you how the thing was wired. This file is the wiring. Every number was
measured in a real browser on 2026-08-15 at 1440x900, or it is marked
`unverified`. The studies stay the source for what those pages look like.
...
A beat with two things changing is two beats, or noise.
...
Apple's page holds thirteen `position: sticky` elements whose containers
measure 1.02, 1.44, 2.00, 2.69, 4.41 and 10.33 viewport heights. The one
carrying the WebGL product viewer is the 2.00: an 1800px container with a
...
the whole beat in one flick and it reads as a twitch. The 2022 iPhone page ran
seven beats over 13.3 screens with several at 1.0 vh, and its own study calls
that the page's weakest property.
...
On igloo.inc and activetheory.net `scrollHeight` equals `innerHeight` (900 =
900) and `scrollY` stays 0 across 19,200 units of wheel delta, with
`touch-action: none` on `<body>`. lusion.co shows the trade most clearly: its
document is 900px tall and its `#ui` element, `position: fixed`, holds
**50,973px** of content, 56.6 viewport heights the scrollbar knows nothing
about. Taking that on means re-implementing keyboard paging, touch momentum,
rubber-band, find-in-page, anchor links and scroll restoration. Three Site of
the Year winners did it. Do not start there.

**Default to native scroll plus sticky.** Reach for ScrollTrigger when a beat
is a timeline rather than a single mapped value, and a smooth-scroll library
(Lenis, Locomotive) for easing without giving up the real document. Reach for
a wheel proxy only when the whole page is one continuous camera move with no
document under it.

## 3. Scroll to value: the maths

Read progress from a rect, never from an offset. `scrollY - track.offsetTop`
is wrong the moment a web font swaps in above the track; a rect is correct on
the first frame, after a scroll-restoring reload, after a hash jump, and
inside any scroller.
...
channel out of step with another. Fizzi does this: beat 3's progress drives
both `can.rotation.y` and an index into a three-word array. Damping has to be
...
requires instead of `scrub: true`. Without the `dt` clamp a backgrounded tab
returns several seconds and the scene snaps in one frame, which is the "it
jumps when I come back" report.

Ease inside a beat, but not on the channel the reader is holding. Rotation
turned by hand should track the hand linearly; easing it reads as lag, not
weight. Camera moves have mass and want `smoothstep(t) = t*t*(3-2*t)`. For a
sequence, `frame = Math.round(p * (FRAMES - 1))`: round, do not floor, because
flooring holds the last frame for one step and drops the first.
...
goes 14,000 to 25,200 with `paused === false`. It plays, gated by an
intersection observer, and pauses on exit. Scrubbing video is the expensive
option and Apple spends it only on the WebGL viewer.

## 4. Pinning that survives

`ambition-tiers.md` lists the five ways a ScrollTrigger pin breaks and the
three ways a scrub breaks. Do not re-derive them. Add these. **Sticky fails
the same way, more quietly:** an ancestor with `overflow: hidden`, `overflow:
auto`, `contain: paint` or any `transform` becomes the containing block, so
the stage sticks inside a box that has already scrolled past, with no error
raised. Assert it at runtime:
...
stage: `hidden` forces the other axis to `auto` and creates a scroll
container.
...
4. Everything inside `gsap.context()`, so one `revert()` removes the tween,
   the trigger, the pin spacer and the inline styles together.
...
7. Verified at 1440, 1024 and 390, and by hand from the bottom of the page up.
...
**GSAP scrub.** `ScrollTrigger` with `pin`, `scrub: 0.8`,
`invalidateOnRefresh`. Cost: about 70KB and the checklist above. Failure mode:
pin spacing, stale measurements, two pins fighting over one range. Choose it
when the beat is a timeline with offsets and overlaps, which is painful to
hand-roll. `demos/horizontal-chapter.html` is the worked example.
...
mode: grey plastic lighting, uncapped DPR, a context never disposed, no answer
when WebGL is missing. Choose it only when the object is the story.
`demos/three-scroll-scene.html` is the worked example.
...
Four more things worth copying. Apple's canvas is **absent from the DOM at
`scrollY` 0** and mounted by `scrollY` 1400, so the first screen never pays
for it. Fizzi runs **one** canvas for a 10.5-screen document, held by
`position: sticky; top: 0`, with each section registering its own trigger
against the shared scene; never mount a canvas per section. For lighting with
no HDRI file, `PMREMGenerator.fromScene(new RoomEnvironment(), 0.04)` ships
inside three.js and gives metal and roughness something to reflect. And frame
on the densest mesh, not on the model's `Box3`: accessory geometry beats the
hero shape in most real scans, and Poly Haven's Camera 01 ships a strap about
three times the size of the body, which put the product at roughly 10% of
frame width until it was corrected.
...
against the text colour: **4.5:1 at the worst of the three, 3:1 for type at
...
first. Igloo's copy never overlaps its object in any of twelve states.
...
- The stage is `100svh`, never `100vh`. On iOS `vh` is the
  address-bar-expanded height, so a `100vh` sticky stage is taller than the
  visible area and its bottom edge is unreachable. The pinned horizontal
  chapter becomes a stack.
- Every still ships `srcset` and a `sizes` matching the real column width. An
  1848px render decoding into a 342px box is a payload the phone never sees.
- Declare it: `<!-- premium-web-design: tier=C mobile=B -->`. The auditor
  checks the claim.

All four WebGL sites ship the full scene to a phone and reframe the camera
rather than simplifying it. Defensible for a studio portfolio, wrong for a
product page with a conversion goal.

## 8. The load, into the first beat

1. **A deadline.** Release the loader on a timer no matter what. One 404, and
   a loader with no upper bound leaves the reader on an empty frame for ever.
2. **A minimum display time, around 550ms.** `demos/three-scroll-scene.html`
   is ready in about 45ms on a warm cache, and a bar that appears and vanishes
...
page, so the screenshot looks right, the console is empty, nobody finds out.
...
Assert on `dataset.scenePath` and on `querySelectorAll('canvas').length`,
never on a screenshot. Two corollaries. A software rasteriser (SwiftShader,
llvmpipe) is a reason to spend less, not to refuse: cap DPR at 1 and render
anyway, or every headless browser that checks your page sees the fallback. And
do not gate your own instrumentation on finding a `<canvas>`. igloo.inc
attaches exactly one shadow root with `mode: "closed"` and builds its canvas
inside it, so `document.getElementsByTagName('canvas').length` is **0** on a
page drawing a full WebGL scene. Its context, captured by patching
...
clinic, a restaurant and a shop have a story with parts, not with movements.
Tier A with real photography beats a half-landed pinned section in front of
any jury. Do not build Tier C without a credible model: a primitive box
standing in for a real product fails this skill outright, so source one with
`/gltf-assets` (Poly Haven is CC0 and needs no key) or stay at Tier B. And do
not add a beat because the page feels short. Three beats that each change one
thing beat six that each change nothing.
